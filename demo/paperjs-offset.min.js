(function (paper) {
    ...
    (function (Offsets) {
         ...
         */
        function ConnectAdjacentBezier(segments1, segments2, origin, mode, offset, limit) {
            ...
            if (origin.isSmooth()) {
                ...
                if (intersection.length === 0) {
                    if (distance > Math.abs(offset) * 0.1) {
                        ...
                        switch (mode) {
                            case 'miter':
                                var join = Intersection(curve1.point2, curve1.point2.add(curve1.getTangentAt(1, true)), curve2.point1, curve2.point1.add(curve2.getTangentAt(0, true)));
                                // prevent sharp angle
         ...
         */
        function ConnectBeziers(rawSegments, mode, source, offset, limit) {
        ...
        function Normalize(path, areaThreshold) {
            if (areaThreshold === void 0) { areaThreshold = 0.0001; }
            if (path.closed) {
        ...
        function OffsetSimple(path, offset, mode, limit) {
            ...
            var segments = Arrayex.Flat(ConnectBeziers(raws, mode, source, offset, limit));
        ...
        Offsets.OffsetSimple = OffsetSimple;
    })(Offsets || (Offsets = {}));
    function OffsetPath(path, offset, mode, limit) {
        var result = path;
        if (path instanceof paper.Path) {
            result = Offsets.OffsetSimple(path, offset, mode, limit);
        }
        else {
            var children = path.children.map(function (c) {
                var offseted = Offsets.OffsetSimple(c, offset, mode, limit);
    ...
    function PrototypedOffset(path, offset, options) {
        options = options || {};
        var offsetPath = OffsetPath(path, offset, options.mode || 'miter', options.limit || 10);
